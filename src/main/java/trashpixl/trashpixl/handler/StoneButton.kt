...
class StoneButton(plugin: Trashpixl?) : Listener {//creating the class
            ...
            if (ev.clickedBlock!!.type == Material.STONE_BUTTON && environment() == 1) {//compare what the player pushed to the stone button
                if (getMinigame() == 4) {//check if the data equals 3
                        ...
                        if (kit == 1) {//checking if kit equals 1
                            ...
                            val item2 = ItemStack(Material.GOLDEN_APPLE, (1..64).random())//creating the item
                            p.inventory.setItem((1..36).random(), item1)//set the item in the inventory
                            p.inventory.setItem((1..36).random(), item2)//set the item in the inventory
                        } else {
                            if (kit == 2) { // checking if kit equals 2
                                val item1 = ItemStack(Material.DIAMOND_CHESTPLATE, 1)//creating the item
                                val item2 = ItemStack(Material.BREAD, (1..64).random())//creating the item
                                p.inventory.setItem((1..36).random(), item1)//set the item in the inventory
                                p.inventory.setItem((1..36).random(), item2)//set the item in the inventory
                            } else {
                                if (kit == 3) { // checking if kit equal 3
                                    ...
                                    if (kit == 4) { // checking if kit equal 4
                                        val item1 = ItemStack(Material.GOLDEN_CHESTPLATE)//creating the item
                                        val item2 = ItemStack(Material.WOODEN_SWORD)//creating the item
                                        p.inventory.setItem((1..36).random(), item1)//set the item in the inventory
                                        p.inventory.setItem((1..36).random(), item2)//set the item in the inventory
                                    } else {
                                        if (kit == 5) { // checking if kit equal 5
                                            val item1 = ItemStack(Material.IRON_BOOTS)//creating the item
                                            val item2 = ItemStack(Material.LEATHER_HELMET)//creating the item
                                            p.inventory.setItem((1..36).random(), item1)//set the item in the inventory
                                            p.inventory.setItem((1..36).random(), item2)//set the item in the inventory
                                        } else {
                                            if (kit == 6) { // checking if kit equal 6
                                                val item1 = ItemStack(Material.CHAINMAIL_LEGGINGS)//creating the item
                                                val item2 = ItemStack(Material.STICK)//creating the item
                                                p.inventory.setItem((1..36).random(), item1)//set the item in the inventory
                                                p.inventory.setItem((1..36).random(), item2)//set the item in the inventory
                                            }
                                        }
